...
const Home = () => {
  ...
  `;
  const contentClass = `
    bg-whiteBackground rounded shadow-lg
    max-w-md mx-auto
  `;
  ...
  return (
    <div className="flex flex-col items-center text-navText font-humanist777 bg-whiteBackground">
      <div
        className={
          isHighRes
            ? "w-full h-fit mb-8 overflow-hidden grid lg:grid-cols-12 md:grid-cols-6 grid-cols-8 lg:gap-8 md:gap-6 gap-2"
            : "flex flex-col items-center mb-8"
        }
      >
        <div></div>
        <div
          className={`w-full max-w-[910px] lg:h-[540px] md:h-[340px] relative lg:col-span-10 md:col-span-4 col-span-6 overflow-hidden ${
            !isMobileXL ? "h-[280px]" : isMobile ? "h-[160px]" : "h-[240px]"
          }`}
        >
          <Carousel
          ...
          {!isPc && (
            <div className="w-72 left-20 p-4 bottom-24 absolute rounded-lg overflow-hidden text-whiteBackground font-humanist777 backdrop-brightness-75 backdrop-blur-sm">
              ...
              <button
                ...
                className="rounded-lg px-[16px] py-[8px] text-white text-xl text-bold font-feltTipRoman bg-[#D93D33] hover:bg-[#DF5D55] focus:bg-[#DF5D55] focus:outline-none drop-shadow-md"
              >
                Mas Información
              </button>
            </div>
      ...
      {products.length > 0 && (
        <p className="mb-8 lg:text-sm text-xs">
          Mira un listado de nuestros productos
        </p>
      )}
      {products.length > 0 && (
        <div
          className={
            isHighRes
              ? "w-full mb-16 grid lg:grid-cols-12 md:grid-cols-6 grid-cols-6 lg:gap-8 md:gap-6 gap-2 items-center"
              : "max-w-[910px] mb-16 flex flex-warp justify-center items-center"
          }
        >
          <div></div>
          <div
            className={`grid lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 lg:gap-8 md:gap-6 gap-2 justify-center lg:col-span-10 md:col-span-4 col-span-4 ${
              isMobileSmall ? "grid-cols-1" : "grid-cols-2"
            }`}
          >
            {products?.map((product, index) => (
              <Product name={product.name} image={product.image} key={index} />
            ))}
          </div>
        </div>
      )}
      <button
        onClick={openModal}
        className="mb-16 rounded-lg px-[16px] py-[8px] text-white lg:text-xl md:text-xl text-lg text-bold font-feltTipRoman bg-[#D93D33] hover:bg-[#DF5D55] focus:bg-[#DF5D55] focus:outline-none drop-shadow-md"
      >
        Quiero registrarme
      </button>
      <Modal
